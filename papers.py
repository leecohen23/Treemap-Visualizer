...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    for row in reader:
        cat_list = str(row[3]).split(":")

        if by_year:
            cat_list = [str(row[2])] + [x.strip() for x in cat_list]
        else:
            cat_list = [x.strip() for x in cat_list]

        curr_data = [row[:3] + row[4:]]

        for key in reversed(cat_list):
            curr_data = [{key: curr_data}]

        list_all_data.append(curr_data[0])

    for data in list_all_data:
        _dict_append(all_data, data, _dict_keys(data), 0)

    return all_data


def _dict_keys(dictionary: Dict) -> List:
    all_key = [key for key in dictionary.keys()]
    if not isinstance(dictionary[all_key[0]][0], Dict):
        return all_key
    else:
        all_key.extend(_dict_keys(dictionary[all_key[0]][0]))
        return all_key


def _dict_append(all_data: Dict, curr_data: Dict, key_list: List,
                 key_num: int) -> None:
    key = key_list[key_num]

    if all_data == {}:
        all_data[key] = curr_data[key]
    elif key not in all_data:
        all_data[key] = curr_data[key]
    elif key in all_data and key_num == len(key_list) - 1:
        all_data[key].append(curr_data[key][0])
    elif key in all_data and isinstance(all_data[key][0], List):
        all_data[key].append(curr_data[key][0])
    elif key in all_data and isinstance(all_data[key][0], Dict):
        _dict_append(all_data[key][0], curr_data[key][0], key_list, key_num + 1)


def _build_tree_from_dict(nested_dict: Union[Dict, List]) -> [PaperTree]:
    """Return a list of trees from the nested dictionary <nested_dict>.
    """
    all_sub_tree = []

    for dict_paper in nested_dict:
        if isinstance(dict_paper, List):
            all_sub_tree.append(
                PaperTree(name=dict_paper[1], subtrees=[],
                          authors=dict_paper[0],
                          doi=dict_paper[3],
                          citations=int(dict_paper[4])))
        else:
            for key in dict_paper:
                all_sub_tree.append(
                    PaperTree(name=key,
                              subtrees=_build_tree_from_dict(dict_paper[key])))
